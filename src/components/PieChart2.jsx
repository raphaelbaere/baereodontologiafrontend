        ...
        try {
            ...
            const treatmenteDataFiltered = treatmentData.filter((treatmentRows) => treatmentRows.realizado === 'Sim');
              ...
              treatmenteDataFiltered.forEach((patient) => {
                const treatmentName = patient.tratamento.nome;
            
                // Use o nome do tratamento como chave para o objeto de contagem
                if (treatmentCounts[treatmentName] === undefined) {
                  treatmentCounts[treatmentName] = 1; // Se ainda não existe uma contagem para este tratamento, inicie com 1
                } else {
                  treatmentCounts[treatmentName] += 1; // Se já existe uma contagem, incremente
                }
              });

              const dataArr = Object.entries(treatmentCounts).map(([name, value], index) => ({
                name,
                value,
